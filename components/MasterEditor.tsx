...
const MasterEditor: React.FC<MasterEditorProps> = ({ isOpen, onClose, settings, isDemoMode = false }) => {
  ...
  return (
    <div className={`fixed inset-0 z-[70] ${isDemoMode ? 'bg-[#0a192f]' : 'bg-[#111318]'} text-onSurface flex flex-col animate-in slide-in-from-bottom duration-200`}>
      {/* Header */}
      <div className={`p-4 border-b border-gray-800 flex justify-between items-center ${isDemoMode ? 'bg-[#0a192f]' : 'bg-[#111318]'} shrink-0`}>
